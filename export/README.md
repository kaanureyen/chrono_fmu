...
4. **`ISO_double_lane_change.txt`**: Sample Bezier path file defining a standard double lane-change maneuver.
5. **`test_terrain.obj`**: Sample Wavefront OBJ 3D mesh file representing a rigid terrain surface.
6. **`circle_100m_left.crg`**: Sample OpenCRG road file for testing roughness and elevation profiles.
7. **`bezier_path_generation.md`**: Guide explaining how to convert 3rd, 5th, and 7th-degree polynomials and discrete coordinate curves into Chrono Bezier paths.
