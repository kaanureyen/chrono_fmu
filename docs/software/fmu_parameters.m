...
terrain_type = 0;            % 0: FlatTerrain, 2: OpenCRG (.crg) (Mesh terrain (.obj) is no longer used)
terrain_crg_file = strrep(fullfile(current_dir, 'default_road.crg'), '\', '/');   % OpenCRG file path (required only if terrain_type = 2)
...
path_file = strrep(fullfile(current_dir, 'default_lane_change_path.txt'), '\', '/');
